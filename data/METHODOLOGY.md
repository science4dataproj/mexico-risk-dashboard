...
| M2/reserves ratio (KLR's best-performing indicator) | ✅ Tested (2026-09) — null result. Cannot be tested against 1994 Tequila at all (M2 data starts 2000); tested against 2008 and 2014, no significant signal in either, after correction. See SERIES_METADATA.md Decisions Log #25. |
...
**The "wrong variable" hypothesis was directly tested, and did not rescue the result.** The M2/international-reserves ratio — the single best-performing leading indicator for currency crises in Kaminsky, Lizondo & Reinhart (1998) — was computed and tested against the same backtest episodes. It could not be tested against 1994 (M2 data starts in 2000, after that crisis), and showed no significant signal against 2008 or 2014 either. This is now three independent, converging null results — the original agnostic CSD backtest (§10 above), the GARCH cross-validation (§9.1), and this literature-directed CSD test — none confirming the bifurcation mechanism, though none with enough episodes to rule it out definitively either. See SERIES_METADATA.md, Decisions Log #25, for the full result table.

**It has now been compared against GARCH, with an informative, mixedresult.** On the reliable `rolling_10y` window, GARCH's conditional-volatility trend agreed with CSD's variance signal on 3 of 4 clean comparisons, with one genuine disagreement (`cetes_28d`). Cross-validated against the same pre-crisis backtest windows used to test CSD (§10), GARCH found no significant signal in any of the 4 valid comparisons after correction — the same null result CSD itself produced, not a divergent one. However, GARCH could only be validly fit in a minority of cases: short-window GARCH estimation is subject to a well-documented small-sample requirement (at least 500 observations for a reliable GARCH(1,1) fit — Hwang & Valls Pereira, 2006), which most of this project's series and windows do not meet. GARCH is therefore not integrated into production — see `SERIES_METADATA.md`, Decisions Log #21-22, for the full comparison.
